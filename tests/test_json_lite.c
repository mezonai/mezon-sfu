...
int main(void) {
  ...
  {
    char out[64];
    int n = sfu_json_extract_string("{\"type\":\"offer\",\"sdp\":\"hello\"}",
                                    "type", out, sizeof(out));
    assert(n == 5);
  ...
  {
    char out[16];
    assert(sfu_json_extract_string("{\"type\":\"offer\"}", "sdp", out,
                                   sizeof(out)) == -1);
  }
  ...
  /* Escape round-trips back to the original through extraction. */
  {
    ...
    char escaped[256];
    int elen =
        sfu_json_escape(original, strlen(original), escaped, sizeof(escaped));
    assert(elen > 0);
    ...
    char roundtrip[256];
    int rlen =
        sfu_json_extract_string(json, "sdp", roundtrip, sizeof(roundtrip));
  ...
  {
    char tiny[4];
    int n = sfu_json_extract_string("{\"sdp\":\"toolong\"}", "sdp", tiny,
                                    sizeof(tiny));
    assert(n == -1);
  }
