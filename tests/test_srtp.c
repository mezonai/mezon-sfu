...
static int build_raw_session(srtp_t *session, const uint8_t *key16, const uint8_t *salt14,
...
int main(void) {
    ...
    assert(build_raw_session(&client_outbound, client_key, client_salt, ssrc_any_outbound) == 0);
    assert(build_raw_session(&client_inbound, server_key, server_salt, ssrc_any_inbound) == 0);
