...
static size_t build_binding_request(uint8_t *buf, const sfu_ice_credentials_t *server_creds,
    ...
    for (int i = 0; i < 12; i++) buf[8 + i] = (uint8_t)(0xA0 + i); /* arbitrary transaction id */
...
int main(void) {
    ...
    size_t response_len = sfu_stun_handle_binding_request(
        request, request_len, &server_creds, &src, sizeof(sin), response, sizeof(response));
    ...
    HMAC(EVP_sha1(), server_creds.pwd, (int)strlen(server_creds.pwd),
         scratch, mi_off, expected_hmac, &expected_len);
    ...
    size_t bad_response_len = sfu_stun_handle_binding_request(
        bad_request, bad_len, &server_creds, &src, sizeof(sin), response, sizeof(response));
