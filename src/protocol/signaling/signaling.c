...
static void handle_offer(int fd, sfu_signaling_server_t *s, const char *sdp, int sdp_len) {
    ...
    int answer_len = sfu_sdp_build_answer(sdp, (size_t)sdp_len,
                                           s->media_host, s->media_port,
                                           s->ice_creds->ufrag, s->ice_creds->pwd,
                                           s->dtls_ctx->fingerprint,
...
static void *conn_thread_main(void *arg) {
    ...
    for (;;) {
        ...
        if (n <= 0) break; /* clean close or error */
        ...
        if (strcmp(type, "offer") == 0) {
            ...
            if (sdp_len < 0) {
                SFU_LOG_WARN("signaling: offer message with no \"sdp\" field, ignoring");
                continue;
            }
            handle_offer(fd, s, sdp, sdp_len);
        } else {
            SFU_LOG_WARN("signaling: unrecognized message type \"%s\", ignoring", type);
        }
    }
