...
static ssize_t read_full_available(int fd, char *buf, size_t cap, const char *terminator) {
    ...
    while (total < cap - 1) {
        ...
        total += (size_t)n;
        buf[total] = '\0';
        if (strstr(buf, terminator)) return (ssize_t)total;
    }
...
ssize_t sfu_ws_recv_text(int fd, char *buf, size_t cap) {
    for (;;) {
        uint8_t header[2];
        if (read_exact(fd, header, 2) < 0) return -1;
        ...
        if (masked) {
            if (read_exact(fd, mask_key, 4) < 0) return -1;
        }
        ...
        if (payload_len > 0 && read_exact(fd, (uint8_t *)buf, payload_len) < 0) return -1;
        if (masked) {
