...
static const char *level_name(sfu_log_level_t level) {
    switch (level) {
        case SFU_LOG_LEVEL_DEBUG: return "DEBUG";
        case SFU_LOG_LEVEL_INFO:  return "INFO ";
        case SFU_LOG_LEVEL_WARN:  return "WARN ";
        case SFU_LOG_LEVEL_ERROR: return "ERROR";
    }
    return "?????";
}
...
void sfu_log(sfu_log_level_t level, const char *file, int line, const char *fmt, ...) {
    ...
    fprintf(stderr, "[%ld.%03ld] %s %s:%d: ",
            (long)ts.tv_sec, ts.tv_nsec / 1000000L, level_name(level), file, line);
