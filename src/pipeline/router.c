...
static void dispatch_record_generation(sfu_vp9_dispatch_diag_t *diag, uint64_t generation) {
  if (!diag) return;
  if (generation == 0) diag->generation_zero++;
  if (diag->generation_min == 0 || generation < diag->generation_min) diag->generation_min = generation;
  if (generation > diag->generation_max) diag->generation_max = generation;
}
#endif
...
static bool forward_local(sfu_worker_t *w, sfu_peer_session_t *sender_session, sfu_ingress_media_t *m, sfu_peer_session_t *sub_session, uint32_t video_ssrc,
  ...
  sfu_egress_media_t media = {
      .publisher = sender_session, .source = m->source, .video_ssrc = video_ssrc, .video_rtx_ssrc = video_rtx_ssrc,
      .remote_slot = remote_slot, .assignment_generation = assignment_generation, .video_pt = video_pt, .video_rtx_pt = video_rtx_pt,
      .has_video = has_video, .is_audio = m->is_audio, .has_svc = m->has_svc, .is_keyframe = m->is_keyframe,
  };
  if (m->has_svc) media.svc = m->svc;
...
#ifdef SFU_DIAG_LOG
    bool ok = forward_local(w, sender_session, m, subscriber, video_ssrc, video_rtx_ssrc, remote_slot, assignment_generation, video_pt, video_rtx_pt, has_video);
    if (diag) { diag->local++; ok ? diag->local_ok++ : diag->local_fail++; }
#else
...
void sfu_router_forward(sfu_worker_t *w, sfu_peer_session_t *sender_session, sfu_ingress_media_t *m) {
...
#ifdef SFU_DIAG_LOG
    routed++;
    if (log_dispatch) diag.recipients++;
#endif
  ...
  if (log_dispatch) {
    ...
    SFU_LOG_WARN("[VP9-DBG] dispatch n=%u worker=%u pub=%u ssrc=%" PRIu32 " seq=%u ts=%" PRIu32 " sid=%u tid=%u kf=%u recipients=%u eligible=%u ineligible=%u invisible=%u invalid_owner=%u local=%u local_ok=%u local_fail=%u remote=%u batches=%u enqueued=%u enqueue_fail=%u copy_fail=%u gen_zero=%u gen=%" PRIu64 "-%" PRIu64,
