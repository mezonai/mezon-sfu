...
bool sfu_paced_send_admit_frame_packet(sfu_paced_send_t *q, uint32_t rtp_timestamp, bool marker, bool keyframe, int64_t now_us) {
  ...
  if (new_frame) {
    if (q->input_frame_active && now_us >= q->input_frame_started_us) {
      ...
      if (span > q->max_input_frame_span_us) q->max_input_frame_span_us = span;
      if (span > SFU_PACED_SEND_MAX_DELAY_US) { q->input_frames_over_delay++; sfu_metric_inc("paced_send_input_frame_slow"); }
    }
    ...
    if (q->drop_input_frame) { q->dropped_delay_frames++; sfu_metric_inc("paced_send_delay_frame_drop"); }
  }
  bool admitted = !q->drop_input_frame;
  if (!admitted) { q->dropped_frame_packets++; sfu_metric_inc("paced_send_frame_packet_drop"); }
  if (marker) { q->input_frame_active = false; q->drop_input_frame = false; }
  return admitted;
}

void sfu_paced_send_reject_input_frame(sfu_paced_send_t *q) {
  if (q && q->input_frame_active) q->drop_input_frame = true;
}
...
bool sfu_paced_send_enqueue(sfu_paced_send_t *q, const uint8_t *data, uint16_t len, const uint8_t *rtx_plaintext, uint16_t rtx_plaintext_len,
  ...
  if (!q || !data || !dst || !metadata || !reservation || !reservation->active || len == 0 || len > SFU_PACED_SEND_MAX_PAYLOAD || rtx_plaintext_len > SFU_PACED_SEND_MAX_PAYLOAD ||
      (rtx_plaintext_len && !rtx_plaintext)) return false;
  if (!q->entries) {
    q->entries = SFU_CALLOC(SFU_PACED_SEND_CAPACITY, sizeof(*q->entries));
    if (!q->entries) return false;
    q->capacity = SFU_PACED_SEND_CAPACITY;
  }
  if (q->count >= q->capacity) { q->dropped_full++; sfu_metric_inc("paced_send_full_drop"); return false; }
  ...
  sfu_paced_send_entry_t *e = &q->entries[q->tail];
  memset(e, 0, sizeof(*e));
  e->release_at_us = base; e->enqueued_at_us = now_us; e->dst = *dst; e->dst_len = dst_len;
  e->len = len; e->rtx_plaintext_len = rtx_plaintext_len; e->pacer_class = pacer_class; e->pacer = pacer; e->reservation = *reservation;
  reservation->active = false; e->metadata = *metadata;
  memcpy(e->data, data, len);
  if (rtx_plaintext_len) memcpy(e->rtx_plaintext, rtx_plaintext, rtx_plaintext_len);
  q->tail = (q->tail + 1u) % q->capacity; q->count++; q->next_release_us = base + span;
  if (q->input_frame_active) q->input_frame_queued_packets++;
  if (q->count > q->high_water) q->high_water = q->count;
  if (release_at_us) *release_at_us = base;
...
static bool entry_valid(const sfu_paced_send_entry_t *e, const sfu_peer_session_t *s) {
  return sfu_session_accepts_work(s) && sfu_session_owner_value(s) == e->metadata.owner_value &&
         ...
         atomic_load_explicit(&s->cold->address_generation, memory_order_acquire) == e->metadata.address_generation &&
...
bool sfu_paced_send_drain(sfu_paced_send_t *q, sfu_worker_t *w, sfu_peer_session_t *s, int64_t now_us) {
  if (!q || !w || !s || !q->entries || q->count == 0) return false;
  bool did_work = false;
  uint32_t processed = 0, sent = 0;
  q->drain_invocations++;
  while (q->count && processed < SFU_PACED_SEND_MAX_DRAIN_PER_SCAN) {
    sfu_paced_send_entry_t *e = &q->entries[q->head];
    if (!entry_valid(e, s)) { cancel_and_pop_entry(q); q->dropped_stale++; processed++; did_work = true; continue; }
    if (e->release_at_us > now_us) break;
    sfu_packet_t *out = sfu_worker_packet_arena_alloc(&w->output_arena);
    if (!out) out = sfu_packet_pool_alloc(w->pp);
    if (!out || e->len > out->cap) { if (out) sfu_worker_release_packet(w, out); break; }
    memcpy(out->data, e->data, e->len); out->len = e->len;
    if (sfu_net_send(w->send_net, out, (const struct sockaddr *)&e->dst, e->dst_len) != 0) {
      sfu_worker_release_packet(w, out); sfu_metric_inc("paced_send_sq_full"); break;
    }
    ...
    if (e->metadata.cache_rtx && sfu_session_video_runtime_ready(s) && s->egress.rtx_cache)
      sfu_rtx_cache_put_stream(s->egress.rtx_cache, e->metadata.subscriber_seq, e->rtx_plaintext, e->rtx_plaintext_len,
                               e->metadata.rtx_ssrc, e->metadata.rtx_pt, e->metadata.media_ssrc, e->metadata.egress_generation);
    ...
    if (residence > q->max_enqueue_to_send_us) q->max_enqueue_to_send_us = residence;
    if (late > q->max_release_late_us) q->max_release_late_us = late;
    q->enqueue_to_send_sum_us += residence; q->enqueue_to_send_samples++;
    sfu_pacer_commit(e->pacer, &e->reservation);
    pop_entry(q); q->sent++; processed++; sent++; did_work = true; sfu_metric_inc("paced_send_sent");
  }
  if (sent > q->max_drain_packets) q->max_drain_packets = sent;
  if (processed == SFU_PACED_SEND_MAX_DRAIN_PER_SCAN && q->count && q->entries[q->head].release_at_us <= now_us) {
    q->drain_cap_hits++; sfu_metric_inc("paced_send_drain_cap_hit");
  }
  if (!q->count) { q->head = q->tail = 0; q->next_release_us = 0; }
  return did_work;
}
