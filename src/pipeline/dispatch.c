  ...
  if (have_ufrag) {
    ...
    if (match) {
      ...
      if (match->has_pending_answer) {
        ...
        pending_is_audience = match->pending_is_audience;
        ...
        if (!session->room) {
          ...
          if (has_pending_answer) {
            ...
            if (!session->is_audience) {
              ...
              SFU_LOG_INFO("worker %u: ufrag=%s joined as audience -- skipping uplink SSRC/PT application (recv-only)", w->worker_index, client_ufrag);
            }
