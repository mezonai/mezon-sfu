...
void sfu_room_touch_peer(sfu_room_t *room, const struct sockaddr_storage *addr,
    ...
    for (uint32_t i = 0; i < room->peer_count; i++) {
        if (room->peers[i].active && addr_equal(&room->peers[i].addr, room->peers[i].addr_len,
                                                  addr, addr_len)) {
    ...
    if (room->peer_count < SFU_ROOM_MAX_PEERS) {
        ...
        SFU_LOG_WARN("room peer table full (%u), dropping new peer", SFU_ROOM_MAX_PEERS);
    }
...
uint32_t sfu_room_list_subscribers_excluding(sfu_room_t *room,
    ...
    for (uint32_t i = 0; i < room->peer_count && n < max_out; i++) {
        if (!room->peers[i].active) continue;
