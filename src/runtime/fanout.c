...
int sfu_fanout_mesh_init(sfu_fanout_mesh_t *mesh, uint32_t worker_count,
    ...
    for (uint32_t i = 0; i < cell_count; i++) {
        ...
        if (sfu_spsc_ring_init(&mesh->rings[i], ring_capacity) != 0) {
            ...
            for (uint32_t j = 0; j < i; j++) sfu_spsc_ring_destroy(&mesh->rings[j]);
            free(mesh->rings);
            sfu_pool_destroy(&mesh->job_pool);
            return -1;
        }
...
static inline sfu_spsc_ring_t *mesh_ring(sfu_fanout_mesh_t *mesh, uint32_t src, uint32_t dst) {
...
bool sfu_fanout_mesh_enqueue(sfu_fanout_mesh_t *mesh, uint32_t src_worker,
                              ...
                              const struct sockaddr_storage *dst_addr, socklen_t dst_len) {
    uint32_t job_idx;
...
unsigned sfu_fanout_mesh_drain(sfu_fanout_mesh_t *mesh, uint32_t dst_worker,
                                unsigned max_count, sfu_fanout_job_fn on_job, void *user_data) {
    unsigned drained = 0;

    for (uint32_t src = 0; src < mesh->worker_count && drained < max_count; src++) {
        if (src == dst_worker) continue; /* diagonal unused */
